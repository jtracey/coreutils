...
```shell
cargo rustc --release --features unix --lib --crate-type dylib
...
To install the dynamically linked library and shim multicall binary:
> [!CAUTION]
> The standard local `/usr/local/bin` path for binaries is typically in the
> default `PATH` environment variable, but the standard local `/usr/local/lib`
> path for libraries is typically *not* in the default library search paths. If
> you do not address this in a persistent manner, then do a default dynamic
> install, your system will likely to fail to fully boot.
```shell
